// Data Connection Lifecycle Management (SignalR-based)
// Note: SignalR handles connections automatically, so this file is simplified
...
// Legacy function - kept for compatibility but SignalR handles connections automatically
export function setupDataConnection(dataConnection, peerId) {
    // SignalR handles connections automatically, so this is a no-op
...
// Legacy function - SignalR handles connections automatically via joinSession
// Kept for backward compatibility
export function attemptConnection(code, retryCount = 0, stopCollaborationFn) {
    // SignalR connections are handled in collaborationCore.js via joinSession
    // This function is kept for compatibility but is essentially a no-op
    // The actual connection is established in joinCollaborationWithCode
    return;
    const maxRetries = 3;
