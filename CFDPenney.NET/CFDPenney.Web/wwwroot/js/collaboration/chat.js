...
// Handle incoming chat message (all messages come from server)
export function handleChatMessage(message, peerId) {
    const senderPeerId = message.peerId || peerId || 'unknown';
    
    // Check for duplicates by messageId if available
    ...
    // Add message to state (all messages come from server)
    const chatMessage = {
        ...message,
        peerId: senderPeerId,
...
// Reactions are handled server-side, no optimistic updates needed
...
function createMessageElement(message) {
    ...
    // Determine if message is from current user by comparing peerId
    const isLocal = message.peerId === state.myPeerId;
    messageDiv.className = `chat-message ${isLocal ? 'chat-message-local' : 'chat-message-remote'}`;
    ...
    if (!isLocal) {
        ...
        nameDiv.textContent = message.name || getDisplayName(message.peerId);
    ...
    reactionBtn.innerHTML = '<i data-lucide="smile" class="w-3 h-3 inline"></i> Add reaction';
...
function showReactionPicker(messageId) {
    ...
    quickReactions.forEach(emoji => {
        ...
        btn.addEventListener('click', (e) => {
            ...
            sendChatReaction(messageId, emoji);
            // Reaction will appear when server broadcasts it back
            removePopup();
        });
